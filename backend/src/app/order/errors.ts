...
export const BranchNotAcceptingOrdersError = new AppError('Branch Not Accepting Orders Currently', 409);
...
export function invalidStatusTransitionError(from: string, to: string) {
  return new AppError(`InvalidStatusTransition: ${from} -> ${to}`, 409);
}
